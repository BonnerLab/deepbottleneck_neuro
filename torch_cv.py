...
def unify_dtypes(*args, target_dtype=None, precision="lowest"):
    ...
    result = tuple(
        arg.clone().to(dtype=target_dtype) if isinstance(arg, torch.Tensor) else arg
        for arg in args
    )
...
class TorchEstimator:
    ...
    def preprocess_data(
        ...
        **kwargs,
        ...
        stats = {
            f"{var}_{stat}": None
            for stat in ["mean", "std", "offset", "scale"]
            for var in ["X", "y"]
        }
        ...
        if output == "offset_scale":
            ...
            if stats["X_offset"] is None:
                stats["X_offset"] = torch.zeros(
                    X.shape[1], dtype=X.dtype, device=X.device
                )
            if stats["y_offset"] is None:
                stats["y_offset"] = torch.zeros(
                    y.shape[1], dtype=y.dtype, device=y.device
                )
            if stats["X_scale"] is None:
                stats["X_scale"] = torch.ones(
                    X.shape[1], dtype=X.dtype, device=X.device
                )
...
class TorchRidgeGCV(TorchEstimator):
    ...
    def fit(self, X, y):
        ...
        if self.store_cv_values:
            self.cv_values_ = torch.empty(
                (*y.shape, n_alphas), dtype=X.dtype, device=X.device
            )
